...
#include "Enregistrement.h"
	...
	class BDD
	{
		...
		public: bool  supprimerEnregistrement(int aId);

				bool viderTable();

		public: Enregistrement* sauvegarderDonneesEnregistrement(Enregistrement *enregistrement,string identifiantCollecteur);

		public: void modifierEnregistrement(int id,int puissance, string albedo, string fluxLum, string idRaspbery, string typeAmpoule, string typeLampadaire, string hauteur, double latitude, double longitude, int altitude, int nbImages, string agglo, string rue, int ulor, string date, string heure, string notes);
