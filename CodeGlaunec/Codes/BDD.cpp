...
std::vector<Enregistrement*> BDD::selectData(string requetesql){
    ...
    if(!rc){
		...
		string albedo,flux, buboid, lamp, streetlight, height, town, street, date, heure, notes;
        ...
        while(rc == SQLITE_OK || rc == SQLITE_ROW){
            ...
            if(rc == SQLITE_ROW){
					...
					Enregistrement *enreg = new Enregistrement(atoi(record.at(0).c_str()), record.at(2), record.at(3), record.at(4), record.at(5), record.at(6), record.at(7), atof(record.at(8).c_str()),  atof(record.at(9).c_str()),atof(record.at(10).c_str()), atoi(record.at(11).c_str()), record.at(12),record.at(13), atoi(record.at(14).c_str()), record.at(15), record.at(16), record.at(17), atoi(record.at(1).c_str()));
                    //Mettre les valeurs dans l'objet enreg
                    //Pour recup (letype) sqlite3_column_text(requete, numdelacolonne)
...
std::vector<Enregistrement*> BDD::obtenirListeDesEnregistrements()
{
	...
	const char *pzTest;
	vector<Enregistrement*> resultat;
	string mon_fichier = "/home/pi/BDD.log";
    ...
    if(!rc){
        ...
        while(rc == SQLITE_OK || rc == SQLITE_ROW){
            ...
            if(rc == SQLITE_ROW){
				...
				vector<string> record;
				for(int i = 0; i < nbColonne+1; i ++) //nbColone = 16 (faire +1 vu que j'ai rajouté l'heure)
				{
				...
				Enregistrement *enreg = new Enregistrement(atoi(record.at(0).c_str()), record.at(2), record.at(3), record.at(4), record.at(5), record.at(6), 
					record.at(7), atof(record.at(8).c_str()),  atof(record.at(9).c_str()),atof(record.at(10).c_str()), atoi(record.at(11).c_str()), record.at(12),record.at(13),
						atoi(record.at(14).c_str()), record.at(15), record.at(16),record.at(17), atoi(record.at(1).c_str()));
      ...
      fichier<< "Base inaccessible : "<< sqlite3_errmsg(db)<<endl;
...
Enregistrement* BDD::sauvegarderDonneesEnregistrement(Enregistrement *enregistrement,string identifiantCollecteur) //test unitaire
{
	...
	std::string sql="INSERT INTO Enregistrements(power,albedo,flux,buboid,lamp,streetlight,height,latitude,longitude,altitude,nbimages,town,street,ulor,date,heure,notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	rc = sqlite3_prepare(db, sql.c_str(), sql.length(), &stmt, &pzTest);
		if(rc != SQLITE_OK)
	{
        	fichier << sql<< "SQL error: "<< sqlite3_errmsg(db);
	}
	if( rc == SQLITE_OK ) 
	{
			// bind the value
		...
		sqlite3_bind_text(stmt, 9,laLongitude.c_str(), laLongitude.length(), 0);
                sqlite3_bind_int(stmt, 10, laAltitude);
		...
		sqlite3_bind_text(stmt,15, laDate.c_str(), 6, 0);
                sqlite3_bind_text(stmt,16, leHeure.c_str(), 6, 0);
...
void BDD::modifierEnregistrement(int id, int puissance, string albedo, string fluxLum, string idRaspbery, string typeAmpoule, string typeLampadaire, string hauteur, double latitude, double longitude,int altitude, int nbImages, string agglo, string rue , int ulor, string date, string heure, string notes)
{
    ...
    const char *pzTest;
			...
			string mon_fichier = "/home/pi/BDD.log";
    ofstream fichier(mon_fichier.c_str(),  ios::out | ios::app  );
	fichier<<"latitude "<<latitude<<endl;
	std :: string sql = "UPDATE Enregistrements SET power=?, albedo=?, flux=? , buboid=?, lamp=?, streetlight=?, height=?, latitude=?, longitude=?,altitude=?, nbimages=?, town=?, street=?, ulor=?, date=?, heure=?, notes=? where id=?";
	...
	sqlite3_bind_int(stmt,1,puissance);
        ...
        sqlite3_bind_int(stmt,10,altitude);
...
bool BDD::supprimerEnregistrement(int id){
...
BDD::BDD()
{
   ...
   if( rc ){
      ...
      //fprintf(stdout, "Opened database successfully\n");
        fichier<< "Opened database successfully "<<endl;
		...
		if(!countColumn)
			{
				...
				//Crée une table appelée Enregistrements qui a comme parametres toutes les données liées a l'enregistrement si elle existe pas deja.
				 ...
				 if( rc != SQLITE_OK ){
					...
					fichier<< "SQL error:: "<< sqlite3_errmsg(db)<<endl;
					sqlite3_free(zErrMsg);
				}
			}
	}
